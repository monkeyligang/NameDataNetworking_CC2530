...
/*************************************************************************************/
//NDN所有包结构
typedef struct InterestName
{
//Interest固定位
//DestinationID标识目的节点的ID号
  uint16	DestinationID;
  //DataFlag标志需要请求的数据包的类型，温度=1，湿度=2，光照=3，电压=4
  uint16	DataFlag;
  //PackageCounter标识包的批次号
  uint16	PackageCounter;
}InterestName;

typedef struct Interest
{
//PackageFlag标识包的类型，interest=1,data=2,其他=3
  uint16	PackageFlag;
  //Interest Name定义
  InterestName	interestName;
  //LastID上一跳的ID号，用于CS表
  uint16	LastID;
  //Noce标识1个随机值，标识是否是重传包
  uint16	Noce;
}Interest;
...
{
//数据包
//PackageFlag标识包的类型，interest=1,data=2,ack=3,其他4
  uint16	PackageFlag;
//Interest Name定义
  InterestName	interestName;
  //数据包变化段
...
//CS表的每一个表项
typedef struct CSItem
{
//CS表的每一个条目的interestname
  InterestName	interestName;
  //CS表中存储DATA包中的本批次数据包的总个数
  uint16   PackageAll;
  //CS表中存储DATA包中的本包属于本批次的第几个包
  uint16   PackageSeg;
  //CS表的每一个条目对应的数据
  uint16   Data;
  //CS表的每一个条目最近被使用的次数
...
//PIT表的每一个表项
typedef struct PITItem
{
  //PIT表的每一个条目的interestName
  InterestName	interestName;
  //PIT表中每一个条目的interestName分别来自哪些节点 
  uint16    IncomingQueue[5];
  //PIT表中每一个条目的interestName已经使用的空间
  uint16    IncomingNumber;
  //PIT表中每一个条目已经存在的时间，用于替换处理时使用
  uint16    time;
}PITItem;

//FIB表的每一个表项
typedef struct FIBItem
{
  //FIB表的每一个条目的interestName
  InterestName	interestName;
  //FIB表中每一个条目的interestName发送到哪个节点
  uint16	OutcomingQueue[5];
  //FIB表中每一个条目的interestQueue已经使用的空间
  uint16	OutcomingNumber;
  //每一个条目已经存在的时间，用于替换处理时使用
  uint16   time;
  //每一个条目相似匹配后使用的次数
  uint16   used;
}FIBItem;
