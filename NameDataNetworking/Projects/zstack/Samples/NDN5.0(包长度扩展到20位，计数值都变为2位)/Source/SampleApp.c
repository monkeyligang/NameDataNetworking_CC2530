...
void SampleApp_SendDataFromCS( uint16 num,uint16 Addr);
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  switch ( pkt->clusterId )
  {
    ...
    case SAMPLEAPP_DATA_CLUSTERID:
      if( BUILD_UINT16(pkt->cmd.Data[14],pkt->cmd.Data[15]) == 0 )
      {
...
{
  uint8 interestData[20];
  interestData[0] = 0x7E;
  interestData[1] = 0x45;
  interestData[2] = 1;//packageflag,interest为1，data为2，其他为3
  uint16 Desid = rand()%6+1; 
  interestData[3] = HI_UINT16(Desid);//interestName,DestinationID
  interestData[4] = LO_UINT16(Desid);//interestName,DestinationID
  interestData[5] = 4;//interestName,DataFlag，温度=1，湿度=2，光照=3，电压=4
  interestData[6] = HI_UINT16(packagecount);//interestName,PackageCounter,包的批次号
  interestData[7] = LO_UINT16(packagecount);
  interestData[8] = HI_UINT16(NodeId);//lastID;
  interestData[9] = LO_UINT16(NodeId);
  interestData[10] = rand()%20;//Noce,随机值
  interestData[11] = 0;//保留位
  interestData[12] = layer;//层数 
  interestData[13] = 0;
  interestData[14] = 0;
  interestData[15] = 0;
  interestData[16] = 0;
  interestData[17] = 0;
  interestData[18] = 0;
  interestData[19] = 0x7E;
  
  HalUARTWrite(0, interestData, 20);
...
{
    if( pkt->cmd.Data[2]==1 && ( pkt->cmd.Data[12] == layer-1) )//兴趣包，并且是上一层下发的兴趣包
    {
       //HalUARTWrite(0, pkt->cmd.Data, pkt->cmd.DataLength); //输出接收到的数据
       AcceptInterest.PackageFlag = 1;
       AcceptInterest.interestName.DestinationID=BUILD_UINT16(pkt->cmd.Data[3],pkt->cmd.Data[4]);
       AcceptInterest.interestName.DataFlag=pkt->cmd.Data[5];
       AcceptInterest.interestName.PackageCounter=BUILD_UINT16(pkt->cmd.Data[6],pkt->cmd.Data[7]);
       AcceptInterest.LastID=BUILD_UINT16(pkt->cmd.Data[8],pkt->cmd.Data[9]);
       AcceptInterest.Noce=pkt->cmd.Data[10];
...
void SampleApp_Ack_Process( afIncomingMSGPacket_t *pkt )
{
   ...
   {
      uint8 result;
      uint8 FIB_ID =BUILD_UINT16(pkt->cmd.Data[3],pkt->cmd.Data[4]);
...
void SampleApp_Data_Process( afIncomingMSGPacket_t *pkt )
{
    ...
    AcceptData.interestName.DestinationID= BUILD_UINT16(pkt->cmd.Data[3],pkt->cmd.Data[4]);
    AcceptData.interestName.DataFlag= pkt->cmd.Data[5];
    AcceptData.interestName.PackageCounter=BUILD_UINT16(pkt->cmd.Data[6],pkt->cmd.Data[7]);
    AcceptData.PackageAll= BUILD_UINT16(pkt->cmd.Data[8],pkt->cmd.Data[9]);
    AcceptData.PackageSeg= BUILD_UINT16(pkt->cmd.Data[10],pkt->cmd.Data[11]);
    AcceptData.Data= BUILD_UINT16(pkt->cmd.Data[12],pkt->cmd.Data[13]);//电压值
...
void SampleApp_ContinueSend(void)
{
  ...
  interestData[3] = HI_UINT16(AcceptInterest.interestName.DestinationID);
  interestData[4] = LO_UINT16(AcceptInterest.interestName.DestinationID);
  interestData[5] = AcceptInterest.interestName.DataFlag;
  interestData[6] = HI_UINT16(AcceptInterest.interestName.PackageCounter);
  interestData[7] = LO_UINT16(AcceptInterest.interestName.PackageCounter);
  interestData[8] = HI_UINT16(NodeId);//lastID;
  interestData[9] = LO_UINT16(NodeId);
  interestData[10] = AcceptInterest.Noce;
  interestData[11] = 0;//保留位
  interestData[12] = layer;//层数 
  interestData[13] = 0;
  interestData[14] = 0;
...
void ForwardingInterestFromFIB(int id)
{
   ...
   {	
        interestData[8] = HI_UINT16(ForwardingTable[id].OutcomingQueue[id]);//lastID;
        interestData[9] = LO_UINT16(ForwardingTable[id].OutcomingQueue[id]); 
       if ( AF_DataRequest( &SampleApp_Periodic_DstAddr, &SampleApp_epDesc,
                             SAMPLEAPP_INTEREST_CLUSTERID,
                             20,
                             interestData,
...
void SendDataFromPIT( uint8 num, Data data)
{
   ...
   //name
   ddata[3] = HI_UINT16(data.interestName.DestinationID);
   ddata[4] = LO_UINT16(data.interestName.DestinationID);
   ddata[5] = data.interestName.DataFlag; //DataFlag标志需要请求的数据包的类型，温度=1，湿度=2，光照=3，电压=4
   ddata[6] = HI_UINT16(data.interestName.PackageCounter); //包批次号
   ddata[7] = LO_UINT16(data.interestName.PackageCounter);
   ddata[8] = HI_UINT16(data.PackageAll);//PackageAll数据包的总个数
   ddata[9] = LO_UINT16(data.PackageAll);
   ddata[10] = HI_UINT16(data.PackageSeg); //PackageSeg目前属于第几个包
   ddata[11] = LO_UINT16(data.PackageSeg);
   ddata[12] = HI_UINT16(data.Data);
   ddata[13] = LO_UINT16(data.Data);
   ddata[16] = 0;
   ddata[17] = 0;
   ddata[18] = 0;
   ddata[19] = 0x7e;
   ...
   {
      ddata[14] = HI_UINT16(PendingTable[num].IncomingQueue[j]);
      ddata[15] = LO_UINT16(PendingTable[num].IncomingQueue[j]);
      if ( AF_DataRequest( &SampleApp_Periodic_DstAddr, &SampleApp_epDesc,
...
void SampleApp_SendDataFromCS( uint16 num,uint16 Addr)
{
   ...
   CSData[1] = 0X45;
   CSData[2] = 2;
   CSData[3] = HI_UINT16(ContentStore[num].interestName.DestinationID);
   CSData[4] = LO_UINT16(ContentStore[num].interestName.DestinationID);
   CSData[5] = ContentStore[num].interestName.DataFlag;
   CSData[6] = HI_UINT16(ContentStore[num].interestName.PackageCounter);
   CSData[7] = LO_UINT16(ContentStore[num].interestName.PackageCounter);
   //CS存储其他内容
   CSData[8] = HI_UINT16(ContentStore[num].PackageAll);
   CSData[9] = LO_UINT16(ContentStore[num].PackageAll);        
   CSData[10] = HI_UINT16(ContentStore[num].PackageSeg);
   CSData[11] = LO_UINT16(ContentStore[num].PackageSeg);
   CSData[12] = HI_UINT16(ContentStore[num].Data);
   CSData[13] = LO_UINT16(ContentStore[num].Data);
   CSData[14] = HI_UINT16(Addr);
   CSData[15] = LO_UINT16(Addr);
   CSData[16] = 0;
   CSData[17] = 0;
   CSData[18] = 0;  
   CSData[19] = 0x7E;
...
void SampleApp_AckMessage_Send(int lastid)
{
  ...
  AckData[1] = 0x45;
  AckData[2] = 3;//Ack为3
  AckData[3] = HI_UINT16(NodeId);
  AckData[4] = LO_UINT16(NodeId);
  AckData[5] = HI_UINT16(lastid);
  AckData[6] = LO_UINT16(lastid);
  AckData[7] = 0;
  AckData[8] = 0;
  AckData[9] = 0;
  AckData[10] = 0;
...
void SampleApp_DataMessage_Send(int id,int counter)
{
   ...
   //name
   data[3] = HI_UINT16(NodeId);
   data[4] = LO_UINT16(NodeId);
   data[5] = 4; //DataFlag标志需要请求的数据包的类型，温度=1，湿度=2，光照=3，电压=4
   data[6] = HI_UINT16(counter); //包批次号
   data[7] = LO_UINT16(counter);
   data[8] = HI_UINT16(num);//PackageAll数据包的总个数
   data[9] = LO_UINT16(num);
   data[10] = HI_UINT16(num); //PackageSeg目前属于第几个包
   data[11] = LO_UINT16(num);
   uint16 voltage = getADC();
   data[12] = HI_UINT16(voltage);
   data[13] = LO_UINT16(voltage);
   data[14] = HI_UINT16(id);//数据包的目的id
   data[15] = LO_UINT16(id);
   data[16] = 0;
   data[17] = 0;
   data[18] = 0;
   data[19] = 0x7e;
