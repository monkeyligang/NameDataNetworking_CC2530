...
void SampleApp_DataMessage_Send(int id);//回数据包
...
void SampleApp_COOR_Send_Message(void)
{
  ...
  interestData[5] = 1;//interestName,PackageCounter,包的批次号
  interestData[6] = NodeId;//lastID;
  interestData[7] = 1;//Noce,随机值
...
void SampleApp_Interest_Process(afIncomingMSGPacket_t *pkt)
{
    if( pkt->cmd.Data[2]==1 && ( pkt->cmd.Data[10] == layer-1) )//兴趣包，并且是上一层下发的兴趣包
    {
       ...
       {
          //若是给自己，直接给上一跳节点回数据
           SampleApp_DataMessage_Send(AcceptInterest.LastID);
       }
       else
       {
          //CS表中查找
...
//直接回数据包
void SampleApp_DataMessage_Send(int id)
{
   uint8 data[12];
   data[0] = 0x7E;
   data[1] = 0x45;
   ...
   data[5] = 1; //包批次号
   data[6] = 1;//PackageAll数据包的总个数
   data[7] = 1; //PackageSeg目前属于第几个包
